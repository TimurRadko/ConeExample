...
public class VolumeSpecificationTest {
    ...
    public void testSpecifiedShouldReturnCorrectListWhenListConsistEntity() {
        Specification specification = new VolumeSpecification(CORRECT_MIN_SURFACE_AREA, CORRECT_MAX_SURFACE_AREA);
        Assert.assertTrue(specification.specified(cone));
    }

    @Test
    public void testSpecifiedShouldReturnEmptyListWhenMinVolumeLessThanExpected() {
        Specification specification = new VolumeSpecification(INCORRECT_MIN_SURFACE_AREA, CORRECT_MAX_SURFACE_AREA);
        Assert.assertFalse(specification.specified(cone));
    }

    @Test
    public void testSpecifiedShouldReturnEmptyListWhenMaxVolumeLessThanExpected() {
        Specification specification = new VolumeSpecification(CORRECT_MIN_SURFACE_AREA, INCORRECT_MAX_SURFACE_AREA);
        Assert.assertFalse(specification.specified(cone));
    }
}
